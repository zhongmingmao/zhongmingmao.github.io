...
> **实测注记（2026-09-08，沙箱 governance-demo，Nacos 原生路径）**：双副本杀一 + ~25 QPS 高压 + 主动通知关闭 = **1230/0 异常**。指标是"HTTP 200 且响应体含 `C[ip]` 段"的**体断言**，并用负对照自证有效性（全 C 缩 0 → 1010 笔中 **230 硬 500**，2 秒内开始）——状态码口径会假绿（demo 无 fallback，之前的全绿都是多副本/泳道回退的真实兜底）。结论：**Nacos 路径被动方案零损失，主动通知无增量**（前文理论预测的实验确认）。另从 4000 事件 ExtraInfo 拿到审计五元组：readOnly 广播 **t0+0.3s**、SC/Dubbo 双通道反注册 SUCCESS **t0+1.2s**——标记与广播几乎同时刻生效。稀疏流量 / Eureka 慢传播两个增量场景沙箱不可测，悬置（forge 应用实测）
...
两阶段预热时长均精确 +120s。**部署即踩的前置坑**：**55199 端口随无损上线开启才监听**——只换探针不开功能 = 永久 connection refused / NotReady，编排模板必须"开无损上线 + 换门"**成对**执行。推论：生产延迟注册 20s 时，业务口探针与注册的时间差被拉大，FAQ 噪音窗口只会更长——55199 迁移的论据链完整
