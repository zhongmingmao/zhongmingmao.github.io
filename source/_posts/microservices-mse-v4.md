...
4. **推送只到客户端内存，最后一公里在 LB**：Ribbon 的 ServerList 有自己的刷新周期，需要订阅直连；实测 yyy-admin-svr 的 `own-app: 1.14.3` 恰好差 1.14.6 这一档——就算切到 Nacos 原生通道，不升 SDK 也拿不到即时刷新
...
> **实测注记（2026-09-08，沙箱 governance-demo，Nacos 原生路径）**：双副本杀一 + ~25 QPS 高压 + 主动通知关闭 = **1230/0 异常**。指标是"HTTP 200 且响应体含 `C[ip]` 段"的**体断言**，并用负对照自证有效性（全 C 缩 0 → 1010 笔中 **230 硬 500**，2 秒内开始）——状态码口径会假绿（demo 无 fallback，之前的全绿都是多副本/泳道回退的真实兜底）。结论：**Nacos 路径被动方案零损失，主动通知无增量**（前文理论预测的实验确认）。另从 4000 事件 ExtraInfo 拿到审计五元组：readOnly 广播 **t0+0.3s**、SC/Dubbo 双通道反注册 SUCCESS **t0+1.2s**——标记与广播几乎同时刻生效。稀疏流量 / Eureka 慢传播两个增量场景沙箱不可测，悬置（own-app 应用实测）

> **补测（2026-09-09，Eureka 兼容层 readOnly 语义直验）**：对 8761/eureka 走协议直测（注册假实例+心跳撑住 → 显式 DELETE → 轮询读路径）——**注册 3s 可见、删除 2s 消失：MSE 兼容层读路径近实时，不存在社区 readOnlyCacheMap 的 30s 同步**。即上文四级延迟的 **① 级在 MSE 兼容层上 ≈0**——Eureka 路径的真实感知窗口 ≈ 消费端拉取周期（自研 10s+KickOut），比按社区语义推算的好得多；被动通知的"拉一次"在兼容层上拿到的是新鲜数据。附带发现：兼容层对无心跳实例的清理极激进（**2~4s 没收** vs 社区 90s 驱逐）——半途死亡的实例注册残留窗口极短（Day 4 c-gray 僵尸残留"秒自愈"的根因）
...
> **实测注记（2026-09-08，沙箱两形态实拍）**：
> 1. **A 形态**（无自定义 preStop）：默认注入业务容器，命令实抄 `echo stop>/tmp/prestop; wget http://127.0.0.1:54199/offline; sleep 30`——**54199=offline 钩子端口，55199=readiness 端口**，两个 Agent 内置端口别混
> 2. **B 形态**（有自定义 preStop `sleep 40`）：sidecar `gracefulshutdown` 精确 **50m/50Mi**、位于 containers，业务 preStop **原样保留不覆盖**
> 3. **双 preStop 并行启动**（广播/双通道反注册 t0+1.2s 内完成，推翻"串行"预判），但删 Pod 终止总时长实测 **93.6s > grace 90 → SIGKILL**（停机上报 t0+40s 后仍有 ~50s 尾巴，shutdownhook 未跑完）——**官方"建议 90"只算对了 A 形态（单层 30s）；B 形态需 grace ≥ 120**
> 4. **onepilot 注入时会自动把 Pod grace 垫高到 60**（Deploy 模板 30 → Pod 实际 60，实证）——A 形态因此天然有 ~29s 收尾余量、无截断（官方"默认 30 不够"的坑由注入方自己兜底）；但垫底是 60 下限，**不救 B 形态**（40+收尾+30 的需求必须模板层显式设 120）
> 5. **生产现场版（2026-09-09 实勘）**：公司生产 Pod（自定义 preStop 形态）**已经带上 gracefulshutdown sidecar 在跑**——B 形态不是未来时；与生产 grace=60 组合即第 3 条的 SIGKILL 敞口，模板 60→120 是接入整改第一优先项
...
> **实测注记（2026-09-08，沙箱复刻本文档全流程）**：按内容灰度 `Agray→B基线→Cgray` 稳定复现（透传 + 穿越回退一枪验证）；按比例 30% 实测 200 笔 **25.5%**（二项抽样 1.4σ 内）；关闭泳道 **100/100** 回基线；**绕过网关直连 A 恒走基线**——打标只发生在入口应用。两个文档外发现：**① 泳道回退是双向的**——基线节点全部消失时，基线流量会落到灰度节点兜底（灰度期间基线副本绝不能缩 0，否则灰度即全量；反之摘灰度节点对基线零影响）；② 半途启动失败的灰度 Pod（报错被删）注册残留会让"确认匹配关系"短暂显示 2/3，心跳超时后自愈。另：删除泳道重建即可更换路由模式，无需删泳道组
...
> **测试环境实勘（2026-09-08，xxx_ack_test 全部 2898 个 Deployment）**：设置 `spec.minReadySeconds`
> 的仅 **1 个**——`kube-system/ack-koord-manager`（阿里云组件自带，3s，与预热无关），**业务侧 0/2897**；
> 同时**未安装 Kruise/OpenKruise**（无相关 CRD），分批发布路径也缺位；yyy ns 滚动策略普遍
...
> **测试环境实勘（yyy ns 83 个 Deployment）**：`55199/readiness` 使用数 **0**；统一编排模板口径 =
...
**组合风险（比裸跑更危险的形态）**：无损上线开启（延迟注册 20s，生产 xxx-biz-boot 实测值）+ readiness
...
**A' 实测结果（2026-09-08，同应用同配置、唯一变量 = 探针）**：

| 探针 | 注册 → 预热开始 | 现象 |
|---|---|---|
| 业务口 `/b:20002`（现状） | **3s** | 探针请求充当"第一笔外部请求"，提前触发预热上报（FAQ 噪音坐实）；无 Readiness 事件 |
| `/readiness:55199` | **35~61s** | 等真实流量路由到新实例（(t/T)² 权重初期≈0 的直接体现）；**出现"微服务通过K8s的Readiness检测"事件**（Agent 只认打自己端口的探测） |

两阶段预热时长均精确 +120s。**部署即踩的前置坑**：**55199 端口随无损上线开启才监听**——只换探针不开功能 = 永久 connection refused / NotReady，编排模板必须"开无损上线 + 换门"**成对**执行。

**生产实勘修正（2026-09-09，xxx-biz-boot 实例 10.129.x.x）**：预期中"延迟注册 20s × 业务口探针 → 预热早于注册 ~20s"的 FAQ 噪音**未在生产出现**——实测 `注册 → 预热开始 +1s → 预热完成 +120s`。机制修正：**预热开始 = 注册后第一笔真实调用**（消费端衔接快）；FAQ"探针提前触发预热"在 4.7.0 + /health 组合不成立，最合理解释是 Agent 默认忽略 actuator 健康路径（ignored_path env 暗示该问题已收敛）；沙箱阶段 1 的 +3s 同样兼容此解释（原"探针触发"归因系误判）。**55199 的核心论据不受影响**：业务口探针使 K8s 门比注册门早开 ~20s（= 延迟注册时长）——这是门的问题，与事件顺序无关
