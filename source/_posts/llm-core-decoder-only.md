...
1. GPT 是 Decoder-only 架构，采用单向注意力机制，在生成文本时，只考虑前面的上下文
   - GPT 更擅长文本生成，即续写，根据上下生成下文
2. Bert 是 Encoder-only 架构，采用双向注意力机制，可以同时考虑上文和下文的信息
   - Bert 可以被应用在更广泛的 NLP 任务中，如文本分类、情感分析、命名实体识别等
